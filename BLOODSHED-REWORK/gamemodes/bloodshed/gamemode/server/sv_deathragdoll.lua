...
local function make_death_anim(ent, rag, type)
    ...
    if type == "bullet" or type == "slash" then
        timer.Simple(dur+math.Rand(2,8), function()
            if !IsValid(rag) or rag.RagHealth <= 0 or math.random(1,100) > CrawlChance then return end
            play_anim_on_rag(rag, "crawling", 0.5, true)
        end)
    end
    rag.IsDeathRagdoll = true
...
hook.Add("EntityTakeDamage", "MuR_DeathAnimsBrutal", function(ent, dmg)
    if ent.IsDeathRagdoll then
        -- print(ent.RagHealth - dmg:GetDamage(), ent.RagHealth)
    ...
    if ent:IsNPC() or ent:IsPlayer() then
        ...
        elseif ent:IsOnGround() and (ent:IsPlayer() and ent:GetVelocity():Length() > ent:GetWalkSpeed() or ent:IsNPC() and ent:GetIdealMoveSpeed() > 150) then
